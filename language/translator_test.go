...
func TestTranslatorTranslate(t *testing.T) {
	translator := createTranslatorInstance(Map{
		"flowers": Map{
			"rose":   "Rose",
			"lily":   "Lily",
			"tulip":  "Tulip",
	...
	for _, test := range tests {
		translation := translator.Translate(test.path, test.args...)
...
func TestTranslatorPluralize(t *testing.T) {
	...
	for _, test := range tests {
		pluralization := translator.Pluralize(test.path, test.amount)
...
func createTranslatorInstance(data Map) *Translator {
	return NewTranslator("test", data)
}
