...
var (
	...
	binary          = []string{"KiB", "MiB", "GiB", "TiB", "PiB", "EiB", "ZiB", "YiB"}
	shortFormBinary = []string{"K", "M", "G", "T", "P", "E", "Z", "Y"}
)
...
func formatBytes(bytes uint64, base float64, units []string, format string) string {
	...
	return fmt.Sprintf(format, strconv.FormatFloat(amount, 'f', formatPrecision, 64), units[int(index)])
}
